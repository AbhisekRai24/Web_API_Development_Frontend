...
export default function UserSidebar({
    ...
    return (
        <>
            {/* Sidebar */}
            <div
                className={`fixed top-0 right-0 h-screen w-96 bg-[#222740] dark:bg-gray-900 shadow-lg transform transition-transform duration-300 z-50
        ${isOpen ? "translate-x-0" : "translate-x-full"}
      `}
            >
                {/* Header */}
                <div className="flex justify-between items-center p-5 border-b border-gray-700 dark:border-gray-600">
                ...
                <div className="flex flex-col h-[calc(100vh-64px)]">
                    <div className="flex-1 overflow-y-auto p-5 space-y-4">
                        {cart.length === 0 ? (
                            <p className="text-gray-400 text-center mt-10 dark:text-gray-300">Your cart is empty.</p>
                        ) : (
                            cart.map((product) => (
                                <div
                                    key={product._id}
                                    className="flex flex-col border-b border-gray-600 dark:border-gray-700 pb-4"
                                >
                                    <div className="flex gap-4 items-center">
                                        <img
                                            src={getBackendImageUrl(product.productImage)}
                                            alt={product.name}
                                            className="w-20 h-20 object-cover rounded-lg border border-gray-700 dark:border-gray-600"
                                        />
                                        <div className="flex-1">
                                            <h4 className="font-semibold text-white dark:text-white">{product.name}</h4>
                    ...
                    {/* Footer */}
                    {cart.length > 0 && (
                        <div className="p-5 border-t border-gray-700 dark:border-gray-600 bg-[#222740] dark:bg-gray-900 space-y-4">
